...
本地辅助脚本。`generate-icon.swift` 生成 app 图标，`package-app.sh` 构建 release 版本并封装 `.app`。
...
发布包目录。`问-1.0.0.zip` 是已经压缩好的 macOS app 包体。
...
```bash
mkdir -p releases
ditto -c -k --keepParent dist/问.app releases/问-1.0.0.zip
```
...
`releases/问-1.0.0.zip` 是发布包。该包只包含 app 程序、图标和 Markdown/公式渲染资源，不包含本地 API key 配置文件。

发布前已经运行完整自动化测试，覆盖配置读取、消息发送、Markdown/公式渲染和 app 打包结构。测试文件不随最终发布仓库上传。
